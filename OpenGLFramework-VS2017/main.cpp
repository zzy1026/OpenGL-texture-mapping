...
GLint iLocLightPos;
float shininess;
bool magnification = true, minification = true, coordinate = true;
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	glUniform3f(iLocLightPos, light.position.x, light.position.y, light.position.z);
	glUniform3f(iLocViewPos, main_camera.position.x, main_camera.position.y, main_camera.position.z);
	glUniform3f(iLocDiffuse, light.diffuse.x, light.diffuse.y, light.diffuse.z);
	glUniform3f(iLocDirection, light.direction.x, light.direction.y, light.direction.z);
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		...
		if(minification == true) glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR);
		else glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
		if (magnification == true)  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		else  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		if (coordinate == true)
		{
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT);	// set texture wrapping to GL_MIRRORED_REPEAT (default wrapping method)
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_I:
			...
			break;
		case GLFW_KEY_G:
			if (magnification == true) magnification = false;
			else magnification = true;
			break;
		case GLFW_KEY_B:
			if (minification == true) minification = false;
			else minification = true;
			break;
		case GLFW_KEY_V:
			if (coordinate == true) coordinate = false;
			else coordinate = true;
			break;
		default:
			break;
		}
...
void setUniformVariables()
{
	...
	iLocDiffuse = glGetUniformLocation(program, "diffuse");
	iLocDirection = glGetUniformLocation(program, "direction");

	// [TODO] Get uniform location of texture
	iLocTex = glGetUniformLocation(program, "texture1");
